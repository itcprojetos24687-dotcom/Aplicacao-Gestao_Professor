...
public class Tela_Principal {
    ...
    private void initialize() {
            ...
            if (menu.equals("Dashboard") || menu.equals("Formadores") || menu.equals("Inscrições") || menu.equals("Formações") || menu.trim().equals("Qualificacoes")) {
                btnMenu.addActionListener(e -> {
                    ...
                    if (menuKey.equals("Qualificacoes")) {
                        ...
                        carregarQualificacoes(); // Carregar dados ao acessar o painel
                    }
        ...
        // Carregar qualificações ao iniciar
    ...
    private JPanel criarPainelQualificacoes() {
        ...
        // Painel de cadastro com TitledBorder
        ...
        // Botão Eliminar Qualificação
        JButton btnEliminarQualificacao = new JButton("Eliminar");
        btnEliminarQualificacao.setBackground(new Color(220, 53, 69));
        btnEliminarQualificacao.setForeground(BRANCO);
        btnEliminarQualificacao.setFont(new Font("Segoe UI", Font.BOLD, 13));
        btnEliminarQualificacao.setPreferredSize(new Dimension(120, 40));
        btnEliminarQualificacao.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btnEliminarQualificacao.addActionListener(e -> eliminarQualificacaoSelecionada());

        panelBotoesQualificacao.add(btnLimparCampos);
        panelBotoesQualificacao.add(btnGravarQualificacao);
        if (nivelAcesso.equalsIgnoreCase("Administrador") || nivelAcesso.equalsIgnoreCase("Secretaria")) {
            panelBotoesQualificacao.add(btnEliminarQualificacao);
        }

        panelSuperior.add(panelBotoesQualificacao, BorderLayout.SOUTH);

        // Tabela de qualificações com TitledBorder
    ...
    // Método para carregar qualificações do banco de dados
    private void carregarQualificacoes() {
        try {
            // Limpar a tabela
            modelQualificacoes.setRowCount(0);
            
            // Buscar todas as qualificações do banco
            ...
            if (lista != null && !lista.isEmpty()) {
                for (Qualificacao q : lista) {
                    ...
                    modelQualificacoes.addRow(new Object[]{
                        q.getCodigo(),
                        q.getTitulo(),
                        "Informática", // Campo - você pode ajustar conforme sua necessidade
                        "Cv4",         // Nível - ajustar conforme sua necessidade
                        "Fernando Langa", // Coordenador - ajustar conforme sua necessidade
                        "Ativo"
                    });
                }
            }
    ...
    // Método para gravar qualificação no banco de dados
    private void gravarQualificacao() {
        ...
        try {
            // Chamar o controller para cadastrar a qualificação
    ...
    // Método para eliminar qualificação selecionada
    private void eliminarQualificacaoSelecionada() {
        int linhaSelecionada = tableQualificacoes.getSelectedRow();
        if (linhaSelecionada == -1) {
            JOptionPane.showMessageDialog(frame, "Por favor, selecione uma qualificação para eliminar.", "Seleção Necessária", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int codigo = (int) modelQualificacoes.getValueAt(linhaSelecionada, 0);
        String titulo = (String) modelQualificacoes.getValueAt(linhaSelecionada, 1);

        int confirmacao = JOptionPane.showConfirmDialog(frame, 
            "Tem certeza que deseja eliminar a qualificação '" + titulo + "'?", 
            "Confirmar Eliminação", 
            JOptionPane.YES_NO_OPTION);

        if (confirmacao == JOptionPane.YES_OPTION) {
            try {
                boolean resultado = qualificacaoController.apagarQualificacao(codigo);
                if (resultado) {
                    carregarQualificacoes();
                    JOptionPane.showMessageDialog(frame, "Qualificação eliminada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(frame, "Erro ao eliminar qualificação.", "Erro", JOptionPane.ERROR_MESSAGE);
                }
            } catch (ExceptionDao e) {
                JOptionPane.showMessageDialog(frame, "Erro ao eliminar qualificação: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
