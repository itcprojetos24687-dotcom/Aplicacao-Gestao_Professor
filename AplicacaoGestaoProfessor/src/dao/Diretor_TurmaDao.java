...
public class Diretor_TurmaDao {
	...
	public void cadastrarDiretor_Turma(Diretor_Turma dt) throws ExceptionDao {
		...
		try {
			...
			// LOG: Cadastro de diretor de turma
			Usuario u = Seccao.obterUtilizador();
			if (u != null) {
				Logs log = new Logs("INSERT", "Diretor de Turma " + dt.getFomador().getNome() + " foi cadastrado", u);
	...
	public void atualizarDiretor_Turma(Diretor_Turma diretor, int codigo)throws ExceptionDao {
		...
		try {
			...
			// LOG: Atualização de diretor de turma
			Usuario u = Seccao.obterUtilizador();
			if (u != null) {
				Logs log = new Logs("UPDATE", "Diretor de Turma " + diretor.getFomador().getNome() + " foi atualizado", u);
				log.setData(LocalDateTime.now());
				new LogDao().salvar(log);
			}
			//JOptionPane.showMessageDialog(null,"Alterado com sucesso");
